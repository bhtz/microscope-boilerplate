...
namespace Microscope.Boilerplate.Framework.Infrastructure.Persistence;
        ...
        When(x => x.Framework == PersistenceOptions.ENTITY_FRAMEWORK, () =>
        {
            ...
            When(x => x.Adapter is not PersistenceOptions.INMEMORY_ADAPTER, () =>
            {
                RuleFor(x => x.ConnectionString)
                    .NotNull()
                    .NotEmpty()
                    .WithMessage("If persistence adapter is not in-memory, ConnectionString configuration is required");
            });
        });
